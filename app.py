...
# IMPORTANT: This line MUST be here for the site to work
app = Flask(__name__)
...
def get_badge(name):
    # Fallback to initials if the logo is missing
...
def home():
    ...
    for m in matches:
        ...
        # Use logo if available, else use badge
...
def match():
    ...
    if "error" in res:
        return render_template_string(LAYOUT, content='<div class="text-center mt-20"><p class="font-black text-zinc-600 mb-4">SYNCING DATA...</p><a href="/" class="text-green-500 text-xs underline">Back to Hub</a></div>')
    ...
    <div class="flex justify-center items-center gap-10 mb-10">
        <div class="text-center"><img src="{res.get('h_logo', get_badge(res['h_name']))}" class="w-16 h-16 object-contain mb-2"><p class="text-[9px] font-black text-zinc-500 uppercase">{res['h_name']}</p></div>
        <div class="text-zinc-800 font-black italic text-2xl uppercase opacity-20 tracking-tighter">VS</div>
        <div class="text-center"><img src="{res.get('a_logo', get_badge(res['a_name']))}" class="w-16 h-16 object-contain mb-2"><p class="text-[9px] font-black text-zinc-500 uppercase">{res['a_name']}</p></div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-10">
        ...
        <div class="bg-[#0f1218] p-6 rounded-[2rem] border border-white/5 shadow-lg">
            <span class="text-[8px] text-zinc-600 font-black uppercase block mb-2 tracking-widest italic">🔴 High Risk</span>
